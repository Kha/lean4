...
| (pats, rhs)::alts, altsRev' => do
  ...
  | _ => do
    -- rhs ← `(fun %%vars... => %%rhs)
    vars ← vars.mapM $ fun var => `((%%var : Lean.Syntax));
...
private unsafe partial def toPreterm (env : Environment) : Syntax → Except String Expr | stx => match_syntax stx with
| `(%%id:id) => match stx.getArg 0 with
...
| `(fun %%params* => %%body) => do
  ...
  params.foldrM (fun param e => do
    (n, ty) ← match_syntax param with
      | `(%%id:id) => pure (id.getIdAt 0, exprPlaceholder)
      | `(_)       => pure (`_a, exprPlaceholder)
      | `((%%id:id : %%ty)) => do {
        ty ← toPreterm ty;
        pure (id.getIdAt 0, ty)
      }
      | _ => unreachable!;
    pure $ Lean.mkLambda n BinderInfo.default ty (Expr.abstract e #[mkFVar n]))
    body
| `(let %%n:id := %%val; %%body) => do
  let n := n.getIdAt 0;
  val ← toPreterm val;
  body ← toPreterm body;
...
| `(%%fn %%arg) => do
  ...
  arg ← toPreterm arg;
  pure $ mkApp fn arg
| `(if %%con then %%yes else %%no) => do
  toPreterm $ Unhygienic.run `(ite %%con %%yes %%no)
| `(()) => pure $ mkConst `Unit.mk
| `((%%e)) => toPreterm e
| _ => match stx.getKind with
...
e ← toPreterm env stx;
pure $ _root_.dbgTrace (toString stx ++ "\n\n" ++ toString e) $ fun _ => e
