...
{ name, src, fullSrc ? src,
  ...
  roots ? [ name ],
  # Output from `lean --deps-json` on all .lean source files. Persist the corresponding output attribute to a file and pass it back in here to avoid IFD.
  modDepsFile ? null,
...
with builtins; let
  ...
  modDepsFile = args.modDepsFile or mkBareDerivation {
    name = "${name}-deps.json";
    buildCommand = ''
      ${lean-leanDeps}/bin/lean --deps-json ${lib.concatStringsSep " " candidateFiles} > $out
    '';
  };
  modDeps = fromJSON (readFile modDepsFile);
  ...
  buildModAndDeps = mod: modMap: if modMap ? ${mod} || externalModMap ? ${mod} then modMap else
    let
      deps = if modDepsMap.${mod} ? "error"
             then abort "error while parsing imports of ${mod}:\n${modDepsMap.${mod}.error}"
             else map (m: m.module) modDepsMap.${mod}.imports;
