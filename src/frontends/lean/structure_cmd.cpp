...
buffer<name> get_structure_fields(environment const & env, name const & S) {
    ...
    while (is_pi(intro_type)) {
        if (i >= nparams)
            fields.push_back(binding_name(intro_type));
        i++;
...
buffer<name> get_parent_structures(environment const & env, name const & structure_name) {
    buffer<name> parents;
    for (name const & field_name : get_structure_fields(env, structure_name)) {
        if (auto p = is_parent_field(env, structure_name, field_name))
            parents.push_back(*p);
    }
    return parents;
}
...
optional<pair<name, expr>> find_field(environment const & env, name const & structure_name, name const & field_name) {
    ...
    while (is_pi(intro_type)) {
        if (binding_name(intro_type) == field_name)
            return some(mk_pair(structure_name, binding_domain(intro_type)));
        intro_type = binding_body(intro_type);
    }
...
void get_structure_fields_flattened(environment const & env, name const & structure_name, buffer<name> & full_fnames) {
    for (auto const & fname : get_structure_fields(env, structure_name)) {
        ...
        if (auto p = is_parent_field(env, structure_name, fname))
            get_structure_fields_flattened(env, *p, full_fnames);
    }
}

expr mk_proj_app(environment const & env, name const & S_name, name const & fname, expr const & e) {
    auto nparams = std::get<1>(get_structure_info(env, S_name));
    auto proj = mk_explicit(mk_constant(S_name + fname));
    for (unsigned i = 0; i < nparams; i++)
...
optional<expr> mk_base_projections(environment const & env, name const & S_name, name const & base_S_name, expr const & e) {
    ...
    for (name const & fname : get_structure_fields(env, S_name)) {
        if (auto p = is_parent_field(env, S_name, fname)) {
    ...
    */
    ...
    void parse_extends() {
        if (m_p.curr_is_token(get_extends_tk())) {
            ...
            while (true) {
                ...
                if (!m_subobjects && m_p.curr_is_token(get_private_tk())) {
                    m_p.next();
    ...
    optional<unsigned> merge(expr const & parent, name const & fname, expr const & ftype) {
        for (unsigned i = 0; i < m_fields.size(); i++) {
            if (local_pp_name(m_fields[i].m_local) == fname) {
                if (m_ctx.is_def_eq(mlocal_type(m_fields[i].m_local), ftype)) {
                    return optional<unsigned>(i);
                } else {
                    expr prev_ftype = mlocal_type(m_fields[i].m_local);
    ...
    expr mk_field_default_value(name const & full_field_name) {
        return ::lean::mk_field_default_value(m_env, full_field_name, [&](name const & fname) {
                for (field_decl const & d : m_fields) {
                    if (local_pp_name(d.m_local) == fname)
    ...
    void process_extends() {
        ...
        for (unsigned i = 0; i < m_parents.size(); i++) {
            ...
            if (m_subobjects) {
                expr field = mk_local(std::string("to_") + parent_name.get_string(), mk_as_is(parent));
                ...
                for (name const & full_fname : subfield_names) {
                        ...
                        if (m_fields[i].get_name() == fname) {
                            throw elaborator_exception(
                                    parent, sstream() << "invalid 'structure' header, field '" << fname
                                                      << "' from '" << parent_name
                                                      << "' has already been declared with a different type");
                        }
                    }
                    ...
                    // TODO(sullrich): HACK
                    // if (full_fname.get_prefix() == parent_name) {
                        /*type = m_p.env().get(full_fname).get_type();
                        buffer<expr> args;
                        ...
                        type = instantiate(type, base_obj);
                        type = instantiate_rev(type, args.size(), &args[0]);
                        type = mk_as_is(type);*/
                ...
                for (size_t fmap_idx = fmap_start; fmap_idx < fmap.size(); fmap_idx++) {
                    ...
                    name fname = local_pp_name(field.m_local);
                    name full_fname = parent_name + fname;
    ...
    field_decl * get_field_by_name(name const & name) {
        auto it = std::find_if(m_fields.begin(), m_fields.end(), [&](field_decl const & inherited_field) {
            return local_pp_name(inherited_field.m_local) == name;
        });
    ...
    void parse_field_block(binder_info const & bi) {
        ...
        for (auto p : names) {
            auto report_duplicate = [&](bool from_parent) {
                sstream msg;
                msg << "field '" << p.second;
                if (from_parent)
            ...
            if (auto old_field = get_field_by_name(p.second)) {
                if (default_value && is_placeholder(type)) {
                    ...
                    report_duplicate(old_field->m_kind == field_kind::from_parent);
                }
            } else {
    ...
    expr elaborate_field(field_decl & decl, expr const & tmp, tele_elab elab) {
        ...
        if (value && (!decl.m_has_new_default || is_placeholder(decl.get_type()))) {
            ...
            decl.m_local = mk_local(decl.get_name(), let_name(new_tmp), let_type(new_tmp), {});
    ...
    void accumulate_levels(buffer<level> & r_lvls) {
        for (field_decl const & decl : m_fields) {
            level l = get_level(m_ctx, mlocal_type(decl.m_local));
    ...
    void display_fields(std::ostream & out) {
        for (field_decl const & decl : m_fields) {
            out << ">> " << mlocal_name(decl.m_local) << " : " << mlocal_type(decl.m_local);
            if (decl.m_default_val)
    ...
    void include_ctx_levels() {
        ...
        for (field_decl const & f : m_fields) {
            all_lvl_params = collect_univ_params(mlocal_type(f.m_local), all_lvl_params);
            if (f.m_default_val)
    ...
    expr mk_intro_type() {
        ...
        for (field_decl const & decl : m_fields) {
            if (decl.m_kind != field_kind::from_parent || !m_subobjects)
                field_wo_defaults.push_back(decl.m_local);
        }
    ...
    void declare_projections() {
        ...
        for (field_decl const & field : m_fields) {
            if (field.m_kind != field_kind::from_parent || !m_subobjects) {
                name field_name = m_name + mlocal_name(field.m_local);
    ...
    void declare_defaults() {
        for (field_decl const & field : m_fields) {
            if (field.m_has_new_default) {
                ...
                expr type = mlocal_type(field.m_local);
                ...
                name decl_name  = name(m_name + local_pp_name(field.m_local), "_default");
            ...
            if (m_subobjects) {
                if (!m_private_parents[i]) {
                    if (m_attrs.has_class() && is_class(m_env, parent_name)) {
                        ...
                        m_env = add_instance(m_env, m_name + mlocal_name(m_fields[i].m_local), m_prio, true);
                    }
                }
                continue;
            }
