...
def simpleBinders := leading_parser many1 binderIdent >> optType
@[builtinTermParser]
def «forall» := leading_parser:leadPrec unicodeSymbol "∀" "forall" >> (simpleBinders <|> many1 (ppSpace >> bracketedBinder)) >> ", " >> termParser
...
def funSimpleBinders   := withAntiquot (mkAntiquot "simpleBinders" ``simpleBinders) <| atomic (lookahead (many1 binderIdent >> " : ")) >> simpleBinders
...
def basicFun : Parser := leading_parser (withAnonymousAntiquot := false) ppGroup ((funSimpleBinders <|> many1 (ppSpace >> funBinder)) >> " =>") >> ppSpace >> termParser
