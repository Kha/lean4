...
instance : MonadEnv CoreM where
  getEnv := return (← read).asyncEnv.wait
  modifyEnv _ := do
    throw <| .error (← getRef) "`modifyEnv` is no longer implemented in `CoreM`"

instance : MonadAsyncEnv CoreM where
  getAsyncEnv := return (← read).asyncEnv
...
@[inline] def modifyCache (f : Cache → Cache) : CoreM Unit :=
  modify fun ⟨next, ngen, trace, cache, messages, infoState⟩ => ⟨next, ngen, trace, f cache, messages, infoState⟩
...
def SavedState.restore (b : SavedState) : CoreM Unit :=
  modify fun s => { s with messages := b.messages, infoState := b.infoState }
...
instance [MetaEval α] : MetaEval (CoreM α) where
  eval env opts x _ := do
    ...
    let (a, _) ← (withConsistentCtx x).toIO { asyncEnv := .ofEnv env, fileName := "<CoreM>", fileMap := default, options := opts } {}
    MetaEval.eval env opts a (hideUnit := true)
...
def ImportM.runCoreM (x : CoreM α) : ImportM α := do
  let ctx ← read
  let (a, _) ← (withOptions (fun _ => ctx.opts) x).toIO { asyncEnv := .ofEnv ctx.env, fileName := "<ImportM>", fileMap := default } {}
  return a
