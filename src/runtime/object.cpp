...
extern "C" obj_res lean_string_from_utf8_unsafe(obj_arg a) {
    size_t sz = lean_sarray_size(a);
    size_t len = utf8_strlen(lean_sarray_cptr(a), sz);
    size_t rsz = sz + 1;
    obj_res r = lean_alloc_string(rsz, rsz, len);
    memcpy(lean_string_cstr(r), lean_sarray_cptr(a), sz);
    w_string_cstr(r)[sz] = 0;
    lean_dec(a);
    return r;
}

extern "C" obj_res lean_string_to_utf8(obj_arg s) {
