...
let (f, _) ← (tryFinally (PrettyPrinter.ppModule stx) printTraces).toIO { options := Options.empty.set `trace.PrettyPrinter.format debug, fileName := "", fileMap := default } { env := env }
IO.print f
